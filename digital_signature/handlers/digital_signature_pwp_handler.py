...
class DigitalSignatureHandlerPWP:
    def __init__(self):
    ...
    def gate_way_response(self, return_value, return_status, reference_number):
        try:
            ...
            minio_file_path = f"{ds_minio_path}/PWP/{signed_by}/signed/{reference_number}_signed.pdf"
            ...
            if return_status.lower() == "success" and return_value != '':
                ...
                url = f"{app_configurations.MODULE_BASE_URL}/#/epr/pwpApplicationList/success"
                logger.debug(f"PWP redirect URL for landing page----> {url}")
    ...
    def sign_digitally(self, request_json, user_info, request):
        try:
            ...
            response_json = CERTIFICATE_GENERATION().pwp_certificate_generation(company_id=company_id,
            ...
            json_input = {
                ...
                "SUrl": f"{request_domain}{Routes.digital_signature_base_url}{DigitalSignature.gateway_response_pwp}?ngsw-bypass=true",
                "FUrl": "/Error",
                "CUrl": "/Cancel",
            ...
            logger.debug(
                f"------->Url,{request_domain}{Routes.digital_signature_base_url}{DigitalSignature.gateway_response_pwp}")
            ...
            self.update_digital_signature_status(company_id=company_id, application_id=application_id,
                                                 ...
                                                 ref_no=reference_no, signed_by=user_id, application_type=1)
            return final_dict
        except Exception as e:
            logger.error(f"Exception Due to {e}", exc_info=True)
    ...
    def update_digital_signature_status(self, company_id, application_id, request_id, ref_no, signed_by,
        ...
        try:
            ...
            insert_query = f"INSERT INTO dsign_logs " \
                           f"(company_id, application_id, request_id, reference_no, created_by, created_at,application_type) " \
    ...
    def update_application_details(self, ref_no, cert_path):
        try:
            ...
            update_digital_signature = f"update {TableName.pwp_application_requests} " \
            ...
            print(update_digital_signature)
    ...
    def gate_way_response_ulb(self, return_value, return_status, reference_number):
        try:
            logger.info(f"Initiated saving digital signature")
            logger.debug(f"Return value : {return_value}")
            logger.debug(f"Return status : {return_status}")
            ...
            arg4 = os.path.join(directory_path, "Decrypted_Signed_Data.txt")
            digital_singed_file_path = os.path.join(digital_signed_files, f'{reference_number}_signed.pdf')
            minio_file_path = f"{ds_minio_path}/ULB/{signed_by}/signed/{reference_number}_signed.pdf"
            ...
            if return_status.lower() == "success" and return_value != '':
                ...
                S3Utility().upload_file_to_s3(digital_singed_file_path, minio_file_path)
                self.update_application_details_ulb(ref_no=reference_number, cert_path=minio_file_path)
                url = f"{app_configurations.MODULE_BASE_URL}/#/epr/ulbApplicationList"
                logger.debug(f"PWP redirect URL for landing page----> {url}")
    ...
    def sign_digitally_ulb(self, request_json, user_info, request):
        try:
            request_domain = request.headers.get('Origin')
            user_id = user_info['user_id']
            result = self.fetch_logged_in_user_details(user_id=user_id)
            ...
            logger.info("Fetching PDF content")
            response_json = CERTIFICATE_GENERATION().ulb_certificate_generation(company_id=company_id,
                                                                                application_id=application_id,
                                                                                user_id=user_id,
                                                                                reference_no=reference_no,
                                                                                designation=designation)
            ...
            json_input = {
                ...
                "No_of_pages": 0,
                "PreviewRequired": True,
                "SUrl": f"{request_domain}{Routes.digital_signature_base_url}{DigitalSignature.gateway_response_ulb}?ngsw-bypass=true",
                "FUrl": "/Error",
                "CUrl": "/Cancel",
                "ReferenceNumber": reference_no,
                "IsCompressed": False,
            ...
            logger.debug(
                f"------->Url,{request_domain}{Routes.digital_signature_base_url}{DigitalSignature.gateway_response_ulb}")
            with open(arg2, "w") as outfile:
                json.dump(json_input, outfile)
    ...
    def update_application_details_ulb(self, ref_no, cert_path):
        try:
            logger.info("update application status for digital signature")
            company_details = self.fetch_company_details(ref_no=ref_no)
            company_id = company_details.get('company_id')
            ...
            signed_by = company_details.get('created_by')
            request_id = company_details.get('request_id')
            update_digital_signature = f"update {TableName.ulb_application_requests} " \
                                       f"set digital_sign_status = '1', " \
                                       f"cert_path = '{cert_path}', " \
                                       f"signed_by = {signed_by} " \
                                       f"where company_id = '{company_id}'and " \
